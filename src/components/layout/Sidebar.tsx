      ...
      <div className={cn("fixed inset-y-0 left-0 z-40 flex flex-col w-64 bg-card border-r border-border transition-all duration-300 overflow-hidden", isMobile && !isOpen && "-translate-x-full", isMobile && "absolute shadow-xl", !isMobile && !isOpen && "w-16")}>
        ...
        <div className={cn("flex-1 overflow-auto py-3 px-3 scrollbar-thin scrollbar-thumb-primary/20 scrollbar-track-transparent hover:scrollbar-thumb-primary/30", !isOpen && !isMobile && "px-2")}>
          <nav className="space-y-1">
            ...
            <NavItem icon={<img src="/lovable-uploads/8b623f45-b9b4-48ee-a02a-8e8ebd58c7e0.png" alt="Jobs" className="w-4 h-4" />} label={isOpen || isMobile ? "Career Hub" : ""} href="/topics/jobs" active={getActiveState("/topics/jobs")} />
            <NavItem icon={<img src="/lovable-uploads/800e50e9-0765-41ca-9728-eb655c16f679.png" alt="Learn" className="w-4 h-4" />} label={isOpen || isMobile ? "Learning Center" : ""} href="/learn" active={getActiveState("/learn")} />
            <NavItem icon={<img src="/lovable-uploads/b2fe3736-d342-4f1d-b060-8bb25c5271de.png" alt="Glossary" className="w-4 h-4" />} label={isOpen || isMobile ? "Financial Glossary" : ""} href="/glossary" active={getActiveState("/glossary")} />
            <NavItem icon={<img src="/lovable-uploads/83f25885-3df9-41ea-9f73-30dc81a20434.png" alt="Streaks" className="w-4 h-4" />} label={isOpen || isMobile ? "Streaks" : ""} href="/streaks" active={getActiveState("/streaks")} />
          </nav>

          {(isOpen || isMobile) && <div className="mt-6 p-3">
